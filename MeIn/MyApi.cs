    ...
    public class MyApi : IMessageDeal
    {
        ...
        public void LoadFromFile()
        {
            try
            {
                ...
                {
                    string[] items = line.Split(new char[] { ' ' }, 3, StringSplitOptions.None);
                    if (items.Length == 3)
                    {
                        Int64 score;
                        Int64 time;
        ...
        {
            var lines = _meinAll.Select(ele => ele.Key + ' ' + ele.Value.score + ' ' + ele.Value.time);
...
{1}";
        ...
        private string DealMessage(string message, string p1, string p2, string nick)
        {
            ...
            else if (message == "注册")
            {
                ...
                if (_meinAll.ContainsKey(uin))
                {
                    return string.Format(_hasregedStr, nick, _personStr);
                }
                else
                {
                    _meinAll.Add(uin, new meinItem() { score = 0, time = DateTime.Now.Ticks - timespan });
                    SaveToFile();
                    return string.Format(_regokStr, nick, _personStr);
                }
            }
            return null;
        }
